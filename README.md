...
`wxRust API documentation`(dead link)
...
automatic rust binding generator for its `_unsafe`(dead link) low-level binding.
...
Generates `a rustdoc documentation`(dead link) under doc directory.
